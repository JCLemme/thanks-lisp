...
This file lists all defined builtins. Please check \`src/b_builtins.c\` for Lisp-defined functions.
...
for sf in src/b_math.c src/b_io.c src/b_lisp.c src/b_string.c; do
    ...
    grep $sf -e "^Cell" | while read ds; do
        ...
        docsig=$(echo "$ds" | awk -F '// ' {'print $2;'} | awk -F ') ' {'print $1;'}) 
        docstr=$(echo "$ds" | awk -F '// ' {'print $2;'} | awk -F ') ' {'print $2;'}) 

        if [[ -n $name ]]; then
            echo "    $sign;" >> $AUTOFILE
            printf "\`$docsig)\` $docstr\n\n" >> $DOCFILE
